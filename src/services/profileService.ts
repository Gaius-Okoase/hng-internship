...
import type { GenderizeRes, AgifyRes, NationalizeRes, IUser, Sort } from "../types.js";
...
export const getAllProfileService = async (query: QueryOptionsSchema) => {
    ...
    if (query.min_country_probability)filters.country_probability = {$gte: Number(query.min_country_probability)};
    if (query.min_gender_probability) filters.gender_probability = {$gte: Number(query.min_gender_probability)};
...
const naturalQueryParser = async (query: QueryOptionsSchema) => {
    ...
    const seniorWords: string[] = ["senior", "seniors", "elder", "elders", "elderly"];
    ...
    if (words.some(w => teenagerWords.includes(w))) filters.age_group = "teenager";
    ...
    const match = country.find(c => lowQuery.includes(c.country_name.toLowerCase()));
